...
function PeriodSelector({ activeId, onSelect, customFrom, customTo, setCustomFrom, setCustomTo, onApplyCustom }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:2, flexWrap:'wrap' }}>
      <span style={{ color:C.textDim, fontSize:10, fontFamily:T.sans, marginRight:6, letterSpacing:'0.06em', textTransform:'uppercase', fontWeight:500 }}>Período</span>
    ...
    <div ref={cardRef} style={{ padding:'48px 44px 44px', animation:'fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) both', transition:'transform 0.05s ease' }}>
      ...
      <form onSubmit={go} style={{ display:'flex', flexDirection:'column', gap:10 }}>
        <input ref={inputRef} type="password" value={pw} onChange={e=>setPw(e.target.value)}
          onFocus={()=>setFocused(true)} onBlur={()=>setFocused(false)} placeholder="Contraseña"
        ...
        />
        {err && <div style={{ background:C.redDim, border:`1px solid ${C.red}25`, borderRadius:7, padding:'8px 12px', color:C.red, fontSize:11, fontFamily:T.sans }}>Contraseña incorrecta</div>}
      ...
      {/* Hint de roles — puedes eliminar este bloque en producción */}
      <div style={{ marginTop:20, padding:'10px 14px', background:'rgba(255,255,255,0.02)', border:`1px solid ${C.border}`, borderRadius:8 }}>
        ...
        {ROLES.map(r => (
          <div key={r.name} style={{ display:'flex', alignItems:'center', gap:8, padding:'3px 0' }}>
            ...
            <span style={{ color:r.color, fontSize:10, fontFamily:T.mono, fontWeight:500, width:62 }}>{r.name}</span>
            <span style={{ color:C.textDim, fontSize:10, fontFamily:T.sans }}>{r.canDownload ? 'Acceso total + reportes' : `Solo ${r.tabs.join(', ')}`}</span>
          </div>
        ))}
      </div>
    </div>
...
function Dash({ onClose, role }) {
  ...
  // ── Definición de tabs visibles según rol ──
  const ALL_TABS = [
      ...
      <div style={{ padding:'14px 18px', borderBottom:`1px solid ${C.border}`, background:C.surface, flexShrink:0 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: isAdmin ? 12 : 8 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:2 }}>
                ...
                <Tag color={C.orange}>BotGO</Tag>
                <Tag color={C.textSub}>Grupo Ortiz</Tag>
                {/* Role badge */}
          ...
          <div style={{ display:'flex', gap:6, alignItems:'center' }}>
            ...
            <button onClick={onClose} style={{ padding:'6px 10px', borderRadius:7, border:`1px solid ${C.border}`, background:'transparent', color:C.textDim, cursor:'pointer', fontSize:13, transition:'color 0.15s', fontFamily:T.sans }}>✕</button>
          </div>
        </div>
            ...
            <div className="card-hover" style={CARD}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
                ...
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  ...
                  <button onClick={loadLeads} className="btn-base" style={{ padding:'5px 12px', borderRadius:7, fontSize:11, fontWeight:500, background:'rgba(255,255,255,0.04)', border:`1px solid ${C.border}`, color:C.textSub, cursor:'pointer', fontFamily:T.sans }}>↻ Sync</button>
                  ...
                  <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                    ...
                    <tbody>
                      ...
                      {!leads.length && !leadsLoad && (
                        <tr><td colSpan={6} style={{ color:C.textDim, textAlign:'center', padding:40, fontFamily:T.sans, fontSize:12 }}>Sin solicitudes registradas aún</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
