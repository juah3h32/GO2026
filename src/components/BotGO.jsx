...
// ── Detección de producto en texto ────────────────────────────────────────────
function detectarProducto(texto) {
  if (!texto) return null;
  const lower = texto.toLowerCase();
  const lista = [
    'stretch film', 'pelicula stretch', 'película stretch', 'stretch',
    'empaque flexible', 'empaque', 'esquineros', 'esquinero',
    'cuerdas', 'cuerda', 'arpillas', 'arpilla', 'malla',
    'sacos', 'saco', 'rafia', 'flexible'
  ];
...
// ── ✅ FIX: Resolver clave de PDF con fallbacks múltiples ─────────────────────
function resolverPdfData(accionPDF, productoCtx) {
  // 1. Usar lo que mandó el backend directamente
  if (accionPDF && PDF_MAP[accionPDF]) return PDF_MAP[accionPDF];

  // 2. Intentar con contexto local si el backend mandó algo no reconocido
  if (accionPDF && productoCtx && PDF_MAP[productoCtx]) return PDF_MAP[productoCtx];

  // 3. Fallback al catálogo general si el backend indicó PDF pero sin clave válida
  if (accionPDF) return PDF_MAP['general'];

  return null;
}

// ── Renderers de mensajes ─────────────────────────────────────────────────────
    ...
    <div className="msg-actions">
      {waLink && (
        <a href={waLink} target="_blank" rel="noopener noreferrer"
           className="msg-action-btn msg-action-wa">
          <WhatsAppIcon />
      ...
      {pdfData && (
        <a href={pdfData.url} target="_blank" rel="noopener noreferrer"
           className="msg-action-btn msg-action-pdf">
          <PdfIcon />
...
export default function BotGO({ language = 'es' }) {
  ...
  useEffect(() => {
    if (isOpen && viewMode === 'chat' && typeof window !== 'undefined' && window.innerWidth > 768) {
  ...
  const toggleListening = useCallback(() => {
    ...
    rec.onend    = () => {
      ...
      if (voiceTextRef.current.trim()) setTimeout(() => sendMessage(null, voiceTextRef.current, true), 600);
    };
    try { rec.start(); } catch(err) { console.error(err); }
  }, [isListening, stopAudio, t]);

  // ── Envío de mensaje ───────────────────────────────────────────────────────
  const sendMessage = useCallback(async (e = null, textOverride = null, isVoice = false) => {
    ...
    // Actualizar contexto de producto local
    const prodDetected = detectarProducto(text);
    if (prodDetected) productoCtxRef.current = prodDetected;
    ...
    try {
      // Enviar solo role + content al API
      const historyClean = [...messages, userMsg].map(m => ({
        role: m.role,
        content: m.content,
      }));
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: historyClean, isVoice, language: currentLangCode }),
      });
      ...
      // ✅ FIX: Actualizar contexto con producto detectado en la respuesta también
      const prodReply = detectarProducto(replyText);
      if (prodReply && !prodDetected) productoCtxRef.current = prodReply;

      // ✅ FIX: Construir waLink
      const waLink = accionWA
        ? `https://wa.me/524432072593?text=${encodeURIComponent(
            'Hola Grupo Ortiz, me interesa cotizar ' + (productoCtxRef.current || 'sus productos')
          )}`
        : null;

      // ✅ FIX PRINCIPAL: Resolución robusta de PDF con fallbacks
      const pdfData = resolverPdfData(accionPDF, productoCtxRef.current);

      console.log('📎 accionPDF recibido:', accionPDF);
      console.log('📦 pdfData resuelto:', pdfData);
  ...
  }, [input, loading, messages, currentLangCode, viewMode, stopAudio, playAudio, t]);
        ...
        {viewMode === 'voice' && (
          <div className="botgo-voice-interface">
            ...
            <div className="voice-controls">
              ...
              <button className={`voice-control-btn primary-mic ${isListening ? 'active' : ''}`} onClick={toggleListening}>
        ...
        {viewMode === 'chat' && (
          <div className="botgo-chat-interface">
            ...
            <div className="botgo-footer-curve">
              <form onSubmit={(e) => sendMessage(e, null, false)} className="botgo-input-capsule"
