...
export default function BotGO({ language = 'es' }) {
  ...
  useEffect(() => {
    ...
    const onOutside = (e) => { if (isOpen&&chatWindowRef.current&&!chatWindowRef.current.contains(e.target)) { const btn=document.querySelector('.botgo-button'); if (btn&&btn.contains(e.target)) return; handleCloseChat(); } };
  ...
  return (
    <div className={`botgo-container ${isOpen?'open':''}`} style={{ fontFamily: isRTL?'Tahoma, Arial, sans-serif':'inherit' }}>
          ...
          <div className="botgo-voice-interface">
            ...
            <div className="voice-controls">
              ...
              <button
                ...
                aria-label="Cambiar a modo teclado"
              >
                <KeyboardIcon />
              </button>
          ...
          <div className="botgo-chat-interface">
            <div className="botgo-header-clean" style={{ flexDirection: isRTL?'row-reverse':'row' }}>
              ...
              <button
                ...
                aria-label="Volver al modo de voz"
              >
                <BackArrowIcon />
              </button>
            ...
            <div className="botgo-footer-curve">
              <form onSubmit={(e)=>sendMessage(e,null,false)} className="botgo-input-capsule" style={{ flexDirection: isRTL?'row-reverse':'row' }}>
                ...
                <button
                  ...
                  onClick={toggleListening}
                  aria-label={isListening ? 'Detener grabación de voz' : 'Activar micrófono'}
                >
                  <MicIcon isListening={isListening}/>
                </button>
                <input ref={inputRef} className="botgo-input-field" type="text" value={input} onChange={(e)=>setInput(e.target.value)}
      ...
      {/* ✅ aria-label agregado al botón flotante principal */}
      {!isOpen && (
        <button
