...
export default function BotGO({ language = 'es' }) {
  ...
  const toggleListening = async () => {
    ...
    // Pedir permiso de micrófono explícitamente primero
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      stream.getTracks().forEach(t => t.stop()); // solo verificar permiso, no grabar
    } catch (err) {
      alert('Permiso de micrófono denegado. Ve a la configuración del navegador y habilita el micrófono para este sitio.');
      return;
    }
    ...
    rec.onresult = (e) => {
      ...
      if (txt) { setInput(txt); voiceTextRef.current = txt; }
    };
    ...
    rec.onerror = (e) => {
      ...
      if (e.error === 'no-speech') {
        setInput('');  // nada detectado, simplemente limpiar
      } else if (e.error === 'not-allowed' || e.error === 'service-not-allowed') {
        alert('Micrófono bloqueado. Haz clic en el candado de la barra de dirección y permite el micrófono.');
      }
    };
